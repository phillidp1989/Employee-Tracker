...
const initialQuestion = async () => {
  ...
  switch (action) {
    ...
    case "Remove Employee":
      removeEmployee(employeeData, employeeArray);
      break;
...
const removeEmployee = async (employeeData, employeeArray) => {
  ...
  const proceed = await confirmationQ(deleteEmployee);
  if (proceed === "Yes") {
...
const removeRole = async (roleData, roleArray) => {
...
const updateRoleDept = async (roleData, roleArray, dept) => {
  try {
    ...
    await update.updateRoleDepartment(roleResult.id, deptResult.id, roleDeptQ.roleDept, roleDeptQ.updatedRoleDept);
    initialQuestion();
  } catch (error) {
