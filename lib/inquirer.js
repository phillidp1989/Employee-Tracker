...
const initialQuestion = async () => {
  ...
  switch (action) {
    ...
    case "Add Role":
      ...
      break;
    case "Remove Role":
      deleteRole(roleData, roleArray, employeeData);
      break;
    ...
    case "Remove Department":
      deleteDepartment(dept, roleData);
      break;
...
const deleteRole = async (roleData, roleArray, employeeData) => {
  ...
  try {
    ...
    const employeeinRole = employeeData.find(obj => obj.role_id === roleResult.id);
    
    // If an employee has the role being removed, the user cannot continue with the deletion of the record
    if (employeeinRole === undefined) {
    ...
    if (proceed === "Yes") {
      ...
      console.log("\nYou cannot remove this role as it is assigned to active employees\n".brightGreen);       
    }
...
const deleteDepartment = async (dept, roleData) => {
  ...
  try {
    ...
    const hasRoles = roleData.filter(obj => obj.department_id === deptResult.id);
    console.log(hasRoles);
    
    const rolesToBeUpdated = hasRoles.map(obj => obj.title);

    // Check to see whether department being deleted has roles assigned to it
    if (hasRoles === undefined) {

      // Call the confirmationQ function to check that the user wishes to delete the department
      const proceed = await confirmationQ(removedDept);
      if (proceed === "Yes") {
        await remove.removeDept(deptResult.id, removedDept);
      }
    } else {
      console.log(`You cannot remove the ${removedDept} department as it has active roles assigned.\nYou will need to remove or update the following roles before you can delete the department\n`.brightGreen);
      console.table(rolesToBeUpdated);      
    }
